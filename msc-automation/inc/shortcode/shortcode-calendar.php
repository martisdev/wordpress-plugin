    ...
    function get_calendar_day(){
        ...
        if ($MyRadio->RESPOSTA_ROWS>0){                     
            ...
            if ($MyRadio->RESPOSTA_ROWS==1){
                ...
                $Strloop .= '<TR>'
                            . '<td><div id="cal-top"><i class="far fa-calendar"> '.$txtHoraIni.'</i></div></td>';
                switch($type_prg):
                    case TIP_AUTOMATIC:
                        ...
                        if(isset($page->ID)){
                            ...
                            $Strloop .= '<td><h2>'.($txtLabel).' gh h</h2>';
                        }
                        break;
                    case TIP_CONEX_CENTRAL:$Strloop .= '<td><h2>'.__('Desconnection','msc-automation').'</h2>';break;                        
                    case TIP_DIRECTE:
                        $page =  get_page_by_title(html_entity_decode($txtLabel));
                        if(isset($page->ID)){
                            $url_prg = $page->guid;                    
                ...
                $Strloop .=  '<div id="cal-bottom"><i class="fas fa-clock">'.$txtDurada.'</i></div></td>';
            }else{
                while($counter < $MyRadio->RESPOSTA_ROWS):                                                 
                    $type_prg = $list['item'][$counter]['TYPE_PROG'];                                
                    $txtLabel   = htmlentities($list['item'][$counter]['NAME']);
                    $txtDescrip = htmlentities($list['item'][$counter]['DESCRIP']);
                    ...
                    /*switch($type_prg):
                        case 2:break;
                        default:$Strloop .= '<td><h2>'.($txtLabel).'</h2>';break;
                    endswitch;*/
                    $page =  get_page_by_title(html_entity_decode($txtLabel), OBJECT, 'page');                    
                    if(isset($page->ID)){
                        $url_prg = $page->guid;                    
                        $Strloop .= '<td><h2><a href="'.$url_prg.'">'.$txtLabel.'</a></h2>';
                    }else{
            ...
            $StrReturn .= $Strloop;
            $StrReturn .= '</TABLE><p>';
            $StrReturn .= __('The rest of the hours','msc-automation').' :';
            ...
            $StrReturn =  '<h2>'.__('default programming','msc-automation').':</h2>';
            $StrReturn .= '<p>';
            switch ($MyRadio->ProgramacioDefecte){
                case TIP_AUTOMATIC:
                    $StrReturn .= __('Radio musical formula','msc-automation');
        ...
        if ($MyRadio->RESPOSTA_ROWS>0){            
            ...
            switch($list['item'][1]['TYPE_PROG']){
                ...
                case TIP_DIRECTE:                    
                    ...
                    break;
                case TIP_CONEX_CENTRAL:/*__('Desconnexió','msc-automation')*/;break;                
            }            
            ?>
