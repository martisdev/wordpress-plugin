<?php
    ...
    function get_last_played($attributes){         
        ...
        <form action="<?php echo get_permalink() ?>" method=post>            
            ...
            <p align=center><input type=submit name=vot value="<?php _e('Search ...','msc-automation')?>"></p>
        </form>
        <?php
    }
        ...
        if(!isset($_POST['GrupOpc'])){            
            ...
            if ($MyRadio->RESPOSTA_ROWS>0){  
                ...
                <form action="<?php echo $url_form; ?>" method=POST>
                    ...
                    <p align=center><input type=submit  name=vot value="<?php _e('Vote','msc-automation'); ?>"></p>
                </form>
                <?php                
            }else{                
    ...
    function get_search_music($attributes){        
        if(isset($_POST['HoraPrg'])){
            //Programem la cançó            
            
            $HoraPrg = strftime( "%Y-%m-%d %H:%M:%S",$_REQUEST['HoraPrg']);
            ...
            if ($espera == 0){
                ...
                if(isset($_POST['GrupOpc'])){
                    ...
                    $strReturn.= "<tr><TD><input type=radio name=HoraPrg value=$mydate_1>". strftime("%H:%M",$mydate_1). "</TD></tr>\n";                    
                    $strReturn.= "<tr><TD><input type=radio name=HoraPrg value=$mydate_2>". strftime("%H:%M" ,$mydate_2) ."</TD></tr>\n";                    
                    $strReturn.= "<tr><TD><input type=radio name=HoraPrg value=$mydate_3>". strftime("%H:%M" ,$mydate_3) ."</TD></tr>\n";
                    $strReturn.="<input type=hidden  name=TemID value=". $temid ."></table>";
                    $strReturn.="<p align=center><input type=submit  name=vot value=".__('Program','msc-automation')."></p>";									
                    $strReturn.="</form>";
                    echo $strReturn;
                }else{
                    $strsql="";
                    if(isset($_POST['strsql'])){
                        $strsql = urldecode($_POST['strsql']) ;
                    }else{
                        if(isset($_GET['strsql'])){
                            $strsql = $_GET['strsql'];
                            //$strsql = get_query_var('strsql'); 
                        }
                    }     
                    ...
                    if(strlen($strsql)>0 or $str_disc>0) {									                                                   
                        ...
                        if ($MyRadio->RESPOSTA_ROWS> 0){                            
                            ...
                            while($counter < $rows_this_page):                                    
                                ...
                                $strReturn.= "<TR>\n";
                                if( $counter == 0){
                                    $strReturn.= "<TD><input type=radio name=GrupOpc value=".$id." checked></TD>\n";		
                                }else{					
                                    $strReturn.= "<TD><input type=radio name=GrupOpc value=".$id."></TD>\n";
                                }
    ...
    function get_last_albums($attributes){
        ...
        if ($MyRadio->RESPOSTA_ROWS>0){            
            ...
            $strReturn .= "<TABLE class='datatable' align=center>"; 
            $strReturn .= "<TR><TH width=".$img_width."></TH>";           
            $strReturn .= '<TH ></TH></TR>';
    ...
    function get_detail_song($attributes ){  
        ...
        if ($MyRadio->RESPOSTA_ROWS>0){
            ...
            <div  style="padding:20px 20px 20px 20px">                    
                <?php if($img_exist==TRUE){echo '<img class="jp-image" src='.$PathToShowImg.'>';}?>
