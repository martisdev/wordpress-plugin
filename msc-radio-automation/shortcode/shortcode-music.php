...
wp_enqueue_script('handle-list_radia', plugins_url('/../../jquery/refresh_list_radia.js', __FILE__, '', true), array('jquery'), '1.0.0', true);
    ...
    <form action="<?php echo get_permalink() ?>" method=GET>
        <p><label for="dradi"><h4><?php _e('What a song played at...', 'mscra-automation')?></h4></label><br>
            <input type="date" name="dradi" max="<?php echo date("Y-m-d"); ?>" value="<?php echo ($dradi == null) ? current_time('mysql', true) : $dradi; ?>" required></p>
        <p align=center><button class="<?php echo $cssbutton; ?>" type=submit ><?php _e('Search...', 'mscra-automation')?></button></p><br>
    </form>
    <?php
}
    ...
    if ($temid == 0) {
        ...
        if ($MyRadio->RESPOSTA_ROWS > 0) {
            ...
            ?>
                ...
                <p align=center><input class="<?php echo $cssbutton; ?>" type=submit value="<?php _e('Vote music to play', 'mscra-automation');?>"></p><br>
            </form>
            <?php
        } else {
    ...
    if (($hp != 0)) {
        ...
        if ($espera == 0) {
            ...
            if ($temid != 0) {
                ...
                $strReturn .= "<p align=center><<button type=submit >" . __('Send', 'mscra-automation') . "</button></p>";
                $strReturn .= "</form>";
                return $strReturn;
            } else {
                ...
                if (strlen($strsql) > 0 or $str_disc > 0) {
                    ...
                    if ($MyRadio->RESPOSTA_ROWS > 0) {
                        ...
                        $strReturn = '<H3>' . __('No songs found', 'mscra-automation') . '</H3>';
                    }
